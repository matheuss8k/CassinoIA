...
const deal = async (userId, bets) => {
    ...
    if (totalBet <= 0) throw new Error("Aposta inválida");
    ...
    saveGameLog(user._id, 'BACCARAT', totalBet, finalPayout, { 
        winner: simulation.winner, pScore: simulation.pScore, bScore: simulation.bScore 
    }, risk.level, engineAdjustment, user.lastTransactionId).catch(console.error);
