                        ...
                        <div className="bg-slate-950 p-3 relative h-[320px] overflow-hidden">
                            ...
                            <div className="grid grid-cols-3 gap-2 h-full relative z-10">
                                {grid.map((symbolId, i) => {
                                    ...
                                    return (
                                        <div key={i} className={`relative rounded-xl border flex items-center justify-center overflow-hidden transition-all duration-300 ${conf.bg} ${conf.border} ${isSpinning ? 'opacity-80 scale-95 blur-sm' : 'opacity-100 scale-100'} ${!isSpinning && isWinLine ? `ring-2 ring-yellow-400 ${conf.glow} scale-105 z-20 brightness-125` : ''}`}>
