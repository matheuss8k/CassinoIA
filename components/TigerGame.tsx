...
import { Volume2, VolumeX, Sparkles, Gem, Flame, Crown, Zap, Gift, Citrus, Coins, Info, Lock, BrainCircuit, Scan, Activity, BarChart3 } from 'lucide-react';
...
// --- OPTIMIZED SOUND SYNTHESIZER ---
let audioCtx: AudioContext | null = null;

// Return the oscillator to control loops
const playSynthSound = (type: 'spin_start' | 'stop' | 'win_small' | 'win_big' | 'multiplier'): OscillatorNode | null => {
    try {
        ...
        if (type === 'spin_start') {
            // CALMER SOUND: Mechanical Hum with LowPass Filter
            // Disconnect direct connection to add filter
            osc.disconnect();
            
            const filter = ctx.createBiquadFilter();
            filter.type = 'lowpass';
            filter.frequency.value = 600; // Muffles the harshness
            filter.Q.value = 1;

            osc.connect(filter);
            filter.connect(gain);

            osc.type = 'triangle'; // Softer wave than sawtooth
            osc.frequency.setValueAtTime(100, now);
            
            // Subtle pitch modulation for "rolling" effect
            const lfo = ctx.createOscillator();
            lfo.type = 'sine';
            lfo.frequency.value = 8; // 8Hz wobble
            const lfoGain = ctx.createGain();
            lfoGain.gain.value = 10; // +/- 10Hz
            ...
            // Monkey-patch stop to clean up LFO
            ...
            osc2.frequency.linearRampToValueAtTime(1108, now + 0.5);
...
const SYMBOLS: {[key: string]: { icon: React.ReactNode, bg: string, border: string, glow: string }} = {
    'orange': { 
        icon: <Citrus size={36} className="text-orange-400 drop-shadow-[0_0_8px_rgba(251,146,60,0.8)]" />, 
        bg: 'bg-gradient-to-br from-orange-900/40 to-black', 
        border: 'border-orange-500/30',
        glow: 'shadow-orange-500/20'
    },
    'firecracker': { 
        icon: <Flame size={36} className="text-red-500 drop-shadow-[0_0_8px_rgba(239,68,68,0.8)]" />, 
        bg: 'bg-gradient-to-br from-red-900/40 to-black', 
        border: 'border-red-500/30',
        glow: 'shadow-red-500/20'
    },
    'envelope': { 
        icon: <Gift size={36} className="text-rose-500 drop-shadow-[0_0_8px_rgba(244,63,94,0.8)]" />, 
        bg: 'bg-gradient-to-br from-rose-900/40 to-black', 
        border: 'border-rose-500/30',
        glow: 'shadow-rose-500/20'
    },
    'bag': { 
        icon: <Coins size={36} className="text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.8)]" />, 
        bg: 'bg-gradient-to-br from-yellow-900/40 to-black', 
        border: 'border-yellow-500/30',
        glow: 'shadow-yellow-500/20'
    },
    'statue': { 
        icon: <Gem size={36} className="text-emerald-400 drop-shadow-[0_0_8px_rgba(52,211,153,0.8)]" />, 
    ...
    'wild': { 
        icon: <TigerIcon />, 
        bg: 'bg-gradient-to-br from-yellow-600/50 via-yellow-900/50 to-black', 
...
// --- SLOT AI COMPONENT ---
const TigerAI = ({ isSpinning }: { isSpinning: boolean }) => {
    // Inicializa com um valor alto para dar boa impressão
    const [volatility, setVolatility] = useState(94);
    const [status, setStatus] = useState("ANALISANDO PRÓX. CICLO");
    
    useEffect(() => {
        if(isSpinning) {
            // Enquanto gira, RNG aleatório para simular cálculo
            ...
            // Quando PARA de girar (Independente se ganhou ou perdeu na atual)
            // A IA calcula a probabilidade para a PRÓXIMA rodada.
            // Gera um número atraente entre 80% e 98%
            ...
            <div className="bg-slate-900/90 border border-yellow-500/30 rounded-3xl p-6 backdrop-blur-xl shadow-[0_0_30px_rgba(234,179,8,0.1)] relative overflow-hidden group flex-1 flex flex-col justify-between min-h-[500px]">
                ...
                <div className="flex-1 flex flex-col items-center justify-center gap-4">
                    ...
                    <div className="text-center space-y-2 mt-4">
                         <div className={`text-2xl font-black tracking-widest ${isSpinning ? 'text-yellow-400 animate-pulse' : 'text-slate-200'}`}>
                             {volatility}%
                         </div>
                         <div className="text-[10px] text-slate-500 uppercase tracking-widest">
                             {isSpinning ? 'Calculando Probabilidade...' : 'Probabilidade Próx. Rodada'}
                         </div>
                    </div>
                    ...
                    <div className="w-full bg-slate-950/50 p-4 rounded-xl border border-white/5 mt-4">
                        <div className="flex items-center gap-2 mb-2">
                             ...
                             <span className="text-xs font-bold text-slate-300">STATUS DA IA</span>
                        </div>
...
export const TigerGame: React.FC<TigerGameProps> = ({ user, updateUser }) => {
    ...
    const [soundEnabled, setSoundEnabled] = useState<boolean>(true); // Start with Sound Enabled
    const [isFullScreenWin, setIsFullScreenWin] = useState<boolean>(false);
    
    // Audio Ref for stopping loop
    ...
    useEffect(() => {
        ...
        return () => { 
            ...
            // CRITICAL FIX: Stop sound immediately on unmount/navigate
            stopSpinSound();
    ...
    const stopSpinSound = () => {
        if (spinAudioRef.current) {
            try { 
                // Using the monkey-patched stop which handles the ramp down
    ...
    const handleSpin = async () => {
        ...
        if (bet < MIN_BET || bet > MAX_BET) return alert(`Aposta deve ser entre ${MIN_BET} e ${MAX_BET}`);
            ...
            if (response.totalWin > 0) {
                setTimeout(() => {
                    ...
                    if (response.isFullScreen) {
                        ...
                        playSound(response.totalWin > bet * 10 ? 'win_big' : 'win_small');
                    }
                }, 300);
            }
        } catch (e: any) {
    ...
    return (
        <div className="w-full h-full flex flex-col items-center relative overflow-hidden">
            ...
            <div className="flex-1 w-full max-w-[1600px] flex items-center justify-center gap-8 relative p-4 min-h-0 pt-20">
                    ...
                    <div className="bg-slate-900 rounded-[2rem] p-1.5 shadow-[0_0_50px_rgba(0,0,0,0.8)] border border-yellow-600/50 relative overflow-hidden group">
                        ...
                        <div className="relative bg-black rounded-t-[1.5rem] p-3 flex justify-between items-center border-b border-white/5 overflow-hidden">
                            ...
                            <div className="flex items-center gap-2 z-10">
                                ...
                                <span className="text-[10px] font-bold text-yellow-100 uppercase tracking-wider">Multiplicador Ativo</span>
                            </div>
                            <div className="flex items-center gap-2 z-10">
                                <button onClick={() => setSoundEnabled(!soundEnabled)} className="text-slate-500 hover:text-white transition-colors">
                                    {soundEnabled ? <Volume2 size={16} /> : <VolumeX size={16} />}
                                </button>
                                <div className="bg-yellow-500/20 border border-yellow-500/50 px-2 py-0.5 rounded text-yellow-300 text-xs font-bold shadow-[0_0_10px_rgba(234,179,8,0.2)]">
                                    2500X <span className="text-[8px] opacity-70">MAX</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-slate-950 p-3 relative h-[320px] overflow-hidden">
                            ...
                            <div className="absolute inset-0 grid grid-cols-3 divide-x divide-white/5 pointer-events-none z-0">
                            ...
                            <div className="grid grid-cols-3 gap-2 h-full relative z-10">
                                {grid.map((symbolId, i) => {
                                    ...
                                    const isWinLine = winningLines.some(lineIdx => {
                                        const line = [[0,1,2], [3,4,5], [6,7,8], [0,4,8], [2,4,6]][lineIdx];
                                        return line.includes(i);
                                    });

                                    return (
                                        <div key={i} className={`
                                            relative rounded-xl border flex items-center justify-center overflow-hidden transition-all duration-300
                                            ${conf.bg} ${conf.border}
                                            ${isSpinning ? 'opacity-80 scale-95 blur-sm' : 'opacity-100 scale-100'}
                                            ${!isSpinning && isWinLine ? `ring-2 ring-yellow-400 ${conf.glow} scale-105 z-20 brightness-125` : ''}
                                        `}>
                                            {/* Scanline Effect per cell */}
                                            <div className="absolute inset-0 bg-gradient-to-b from-white/5 to-transparent pointer-events-none"></div>
                                            
                                            {/* Spinning Blur Effect Simulation */}
                                            {isSpinning && (
                                                <div className="absolute inset-0 flex flex-col items-center justify-center opacity-60">
                                                     <div className="w-full h-full bg-gradient-to-b from-transparent via-white/10 to-transparent animate-pulse blur-md"></div>
                                                </div>
                                            )}

                                            <div className={`
                                                relative z-10 transform transition-transform duration-300
                                                ${!isSpinning && isWinLine ? 'animate-bounce' : ''}
                                                ${isSpinning ? 'animate-[spin_0.1s_linear_infinite] blur-[2px] opacity-70 translate-y-10' : 'translate-y-0'}
                                            `}>
                                                {conf.icon}
                                            </div>
                                        </div>
                        ...
                        {/* Bottom Info Bar - REMOVIDO POR OPÇÃO DO USUÁRIO */}
                    ...
                    <div className="bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-xl">
                        <div className="flex justify-between items-center mb-4 bg-slate-950/50 p-2 rounded-xl border border-white/5">
                            <div className="text-left px-2">
                                <span className="text-[9px] text-slate-500 uppercase font-bold block mb-0.5">Último Ganho</span>
                                <span className={`font-mono font-bold text-lg ${winAmount > 0 ? 'text-green-400 animate-pulse' : 'text-slate-300'}`}>
                                    {winAmount > 0 ? `R$ ${winAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : 'R$ 0,00'}
                                </span>
                            </div>
                            <div className="h-8 w-[1px] bg-white/10"></div>
                            <div className="text-right px-2">
                                <span className="text-[9px] text-slate-500 uppercase font-bold block mb-0.5">Saldo</span>
                                <span className="font-mono font-bold text-white text-sm">R$ {user.balance.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-3">
                            <div className="flex-none bg-slate-950 rounded-xl p-1.5 flex flex-col items-center justify-between border border-white/5 w-28">
                                ...
                                <input 
                                    ...
                                    max={MAX_BET} 
                                    value={bet} 
                                    onChange={(e) => !isSpinning && setBet(Math.min(MAX_BET, Math.max(0, parseInt(e.target.value) || 0)))}
                                    disabled={isSpinning}
                                    className="w-full bg-transparent text-center font-bold text-white text-lg my-1 outline-none border-b border-white/10 focus:border-yellow-500 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                                />
                                <div className="flex w-full gap-1">
                                    <button onClick={() => !isSpinning && setBet(Math.max(MIN_BET, Math.floor(bet / 2)))} disabled={isSpinning} className="flex-1 bg-slate-800 hover:bg-slate-700 rounded text-slate-300 font-bold text-sm h-6 disabled:opacity-50">½</button>
                                    <button onClick={() => !isSpinning && setBet(Math.min(MAX_BET, bet * 2))} disabled={isSpinning} className="flex-1 bg-slate-800 hover:bg-slate-700 rounded text-slate-300 font-bold text-sm h-6 disabled:opacity-50">2x</button>
                                </div>
                            </div>
                            ...
                            <Button 
                                onClick={handleSpin} 
                                disabled={isSpinning}
                                className={`flex-1 h-auto text-xl rounded-xl border-b-4 border-yellow-700 active:border-b-0 active:translate-y-1 shadow-[0_0_30px_rgba(234,179,8,0.2)] ${isSpinning ? 'opacity-80 cursor-wait grayscale' : 'animate-pulse-gold'}`}
                                variant="primary"
                            >
                                {isSpinning ? (
                                    <span className="flex items-center gap-2 text-sm"><Sparkles className="animate-spin" size={16}/> GIRANDO</span>
                                ) : (
                                    <div className="flex flex-col items-center leading-none gap-1">
                                        <span>GIRAR</span>
                                        <span className="text-[9px] opacity-60 font-normal tracking-wider">AUTO</span>
                                    </div>
                                )}
                            </Button>
                        </div>
                ...
                {/* Right: AI Analysis (Desktop Only) */}
                <div className="hidden xl:flex w-[280px] flex-col gap-4 justify-center shrink-0">
